...
trim <- ThermoData_long %>% 
  ...
  summarize(end = last(Datetime),
            start = end - minutes(10)) %>% 
  mutate(exp_duration = as.duration(interval(start = start, end = end))) %>% 
  drop_na()
...
ThermoData_trimmed <- joint %>% 
  ...
  mutate(timer = 
           as.duration(Datetime - start)) %>% 
  # select(-c(start, end, interval)) %>% 
  filter(Trial == 1)

windlist <- unique(levels(ThermoData_trimmed$WindSpeed))
  
  for (wind in windlist){
    
    plot <- ThermoData_trimmed %>%
      filter(WindSpeed == wind) %>% 
      ggplot(
             aes(x=timer, y=temp, group=as.factor(Wig), color=Wig)) +
      geom_line() +
      facet_wrap(Thermocouple ~ Radiation, nrow = 4, scales = "free_x") +
      theme_bw() +
      ylim(25, 55) +
      labs(title = paste0("Thermocouple data for ", wind, " m/s windspeed"))
    print(plot)
